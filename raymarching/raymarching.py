...
class _near_far_from_aabb(Function):
    ...
    def forward(ctx, rays_o, rays_d, aabb, min_near=0.2):
        ...
        nears = torch.full((N), fill_value=torch.nan, dtype=rays_o.dtype, device=rays_o.device)
        fars = torch.full((N), fill_value=torch.nan, dtype=rays_o.dtype, device=rays_o.device)
...
class _morton3D(Function):
    ...
    def forward(ctx, coords):
        ...
        indices = torch.full((N), fill_value=torch.nan, dtype=torch.int32, device=coords.device)
...
class _packbits(Function):
    ...
    def forward(ctx, grid, thresh, bitfield=None):
        ...
        if bitfield is None:
            bitfield = torch.full((N), fill_value=torch.nan, dtype=torch.uint8, device=grid.device)
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, ts, rays, T_thresh=1e-4, binarize=False):
        ...
        weights_sum = torch.full((N), fill_value=torch.nan, dtype=sigmas.dtype, device=sigmas.device)

        depth = torch.full((N), fill_value=torch.nan, dtype=sigmas.dtype, device=sigmas.device)
